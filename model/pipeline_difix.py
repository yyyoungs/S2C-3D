...
# from pipeline_scheduler import GaussianScheduler
...
def my_vae_encoder_fwd_new(self, sample):
...
def my_vae_decoder_fwd_new(self, sample, latent_embeds=None):
    def create_custom_forward(module):
        def custom_forward(*inputs):
            return module(*inputs)
    ...
    if latent_embeds is None:
        ...
        sample = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(self.conv_norm_out),
                    sample,
                    latent_embeds,
                    use_reentrant=False,
                )
    # sample = self.conv_act(sample)
    sample = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(self.conv_act),
                    sample,
                    use_reentrant=False,
                )
    sample = torch.utils.checkpoint.checkpoint(
...
class DifixPipeline(
    ...
    def set_train(self,lora_rank_vae):
    ...
    def __call__(
        ...
        correct_img: PipelineImageInput = None,
        correct_noise: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        for infer_step in range(num_inference_steps):
        # with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if correct_noise is not None:
                    correction = noise_pred - correct_noise
                DDIM_outputs = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=True)
                latents = DDIM_outputs.prev_sample
                pred_x0 = DDIM_outputs.pred_original_sample
        ...
        if correct_img is not None:
            ...
            if latent_model_input.shape[0] == 1:
                image = image.chunk(2, dim=0)[0]
            image = torch.clamp(image,-1.0,1.0)
            correct_img = correct_img.to(device=image.device)
            B,_,H,W = correct_img.shape
            image = image.repeat(B,1,1,1)
            cor_img = correct_img[:, :3, :, :]
            cor_mask = correct_img[:, 3:4, :, :]
            l1loss = F.l1_loss(cor_img*cor_mask, image*cor_mask)
            norm_grad = torch.autograd.grad(outputs=l1loss, inputs=latent_model_input)[0]
            try:
                rho = (correction * correction).mean().sqrt().item() / (norm_grad * norm_grad).mean().sqrt().item()*0.2
            except:
                rho = 0
            ...
            with torch.no_grad():
                ...
                if ref_image is not None:
                    image = image.chunk(2, dim=0)[0]
                image = torch.clamp(image,-1.0,1.0)
            return image,l1loss.detach().item()
        if save_noise:
...
ArtifactRemovalPipeline = DifixPipeline
